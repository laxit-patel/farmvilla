	...
	if(is_single() || is_singular()){
		...
		function seopress_automatic_rich_snippets_manual_option($id, $schema_name, $post_meta_key, $seopress_pro_schemas, $sp_schemas_dyn_variables, $sp_schemas_dyn_variables_replace) {
			if (!empty($post_meta_key)) {
				foreach ($post_meta_key as $key => $value) {
					...
					if ($post_meta_value == 'manual_single' || $post_meta_value == 'manual_img_single' || $post_meta_value == 'manual_date_single' || $post_meta_value == 'manual_time_single' || $post_meta_value =='manual_rating_single') {
			...
			$html .= '{
					...
					if ($article_img !='') {
						$html .= '"image": {
							...
							"height": '.json_encode($article_img_height).',
							"width": '.json_encode($article_img_width).'
						},';
					}
		...
		function seopress_automatic_rich_snippets_review_option($schema_datas) {
			...
			$html .= '{
				...
				"@type": "Review",';
				if ($review_item) {
					$html .= '"itemReviewed":{"@type":"Thing","name":'.json_encode($review_item);
				}
		...
		if ($post !='') {
			if (get_post_field('post_content', $post->ID) !='') {
				$seopress_get_the_content = wp_trim_words(esc_attr(stripslashes_deep(wp_filter_nohtml_kses(strip_shortcodes(get_post_field('post_content', $post->ID))))), $seopress_excerpt_length);
			} else {
		...
		if (isset($product) && function_exists('get_date_on_sale_from')) {
		...
		/*Date on sale to*/
		$get_date_on_sale_to ='';
		if (isset($product) && function_exists('get_date_on_sale_to')) {
		...
		if (taxonomy_exists('product_cat')) {
			$product_cat_term_list = wp_get_post_terms(get_the_ID(), 'product_cat', array("fields" => "names"));
			if (!empty($product_cat_term_list) && !is_wp_error($product_cat_term_list)) {
				$product_cat_term_list = $product_cat_term_list[0];
			}
		}

		/*regular price*/
		$get_regular_price ='';
		if (isset($product) && function_exists('get_regular_price')) {
			$get_regular_price = $product->get_regular_price();
		}

		/*sale price*/
		$get_sale_price ='';
		if (isset($product) && function_exists('get_sale_price')) {
			$get_sale_price = $product->get_sale_price();
		}

		/*sku*/
		$get_sku ='';
		if (isset($product) && function_exists('get_sku')) {
