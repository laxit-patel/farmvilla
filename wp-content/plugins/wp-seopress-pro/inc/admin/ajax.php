...
if ( is_plugin_active( 'admin-columns-pro/admin-columns-pro.php' )) {
    add_action( 'ac/column_types', 'ac_register_column_sp_title' );
    function ac_register_column_sp_title( AC_ListScreen $list_screen ) {
        if ( 'post' === $list_screen->get_group() ) {
            require_once plugin_dir_path( __FILE__ ) . 'admin-columns/ac-column-sp_title.php';
            $list_screen->register_column_type( new AC_Column_sp_title );
        }
    }

    add_action( 'ac/column_types', 'ac_register_pro_column_sp_title' );
    function ac_register_pro_column_sp_title( AC_ListScreen $list_screen ) {
        if ( 'post' === $list_screen->get_group() ) {
            require_once plugin_dir_path( __FILE__ ) . 'admin-columns/ac-column-sp_title.php';
    ...
    add_action( 'ac/column_types', 'ac_register_column_sp_desc' );
    function ac_register_column_sp_desc( AC_ListScreen $list_screen ) {
        if ( 'post' === $list_screen->get_group() ) {
            require_once plugin_dir_path( __FILE__ ) . 'admin-columns/ac-column-sp_desc.php';
            $list_screen->register_column_type( new AC_Column_sp_desc );
        }
    }

    add_action( 'ac/column_types', 'ac_register_pro_column_sp_desc' );
    function ac_register_pro_column_sp_desc( AC_ListScreen $list_screen ) {
        if ( 'post' === $list_screen->get_group() ) {
            require_once plugin_dir_path( __FILE__ ) . 'admin-columns/ac-column-sp_desc.php';
    ...
    add_action( 'ac/column_types', 'ac_register_column_sp_noindex' );
    function ac_register_column_sp_noindex( AC_ListScreen $list_screen ) {
        if ( 'post' === $list_screen->get_group() ) {
            require_once plugin_dir_path( __FILE__ ) . 'admin-columns/ac-column-sp_noindex.php';
            $list_screen->register_column_type( new AC_Column_sp_noindex );
        }
    }

    add_action( 'ac/column_types', 'ac_register_pro_column_sp_noindex' );
    function ac_register_pro_column_sp_noindex( AC_ListScreen $list_screen ) {
        if ( 'post' === $list_screen->get_group() ) {
            require_once plugin_dir_path( __FILE__ ) . 'admin-columns/ac-column-sp_noindex.php';
    ...
    add_action( 'ac/column_types', 'ac_register_column_sp_nofollow' );
    function ac_register_column_sp_nofollow( AC_ListScreen $list_screen ) {
        if ( 'post' === $list_screen->get_group() ) {
            require_once plugin_dir_path( __FILE__ ) . 'admin-columns/ac-column-sp_nofollow.php';
            $list_screen->register_column_type( new AC_Column_sp_nofollow );
        }
    }

    add_action( 'ac/column_types', 'ac_register_pro_column_sp_nofollow' );
    function ac_register_pro_column_sp_nofollow( AC_ListScreen $list_screen ) {
        if ( 'post' === $list_screen->get_group() ) {
            require_once plugin_dir_path( __FILE__ ) . 'admin-columns/ac-column-sp_nofollow.php';
...
//Request Alexa Rank
///////////////////////////////////////////////////////////////////////////////////////////////////
function seopress_request_alexa_rank() {
    check_ajax_referer( 'seopress_request_alexa_rank_nonce', $_GET['_ajax_nonce'], true );

    delete_transient( 'seopress_results_alexa_rank');
    if ( false === ( $seopress_results_alexa_rank_cache = get_transient( 'seopress_results_alexa_rank' ) ) ) {
        $response = wp_remote_get('https://data.alexa.com/data?cli=10&url='.get_home_url());
        ...
        $seopress_results_alexa_rank_cache = $response_body;
        set_transient( 'seopress_results_alexa_rank', $seopress_results_alexa_rank_cache, 365 * DAY_IN_SECONDS );
    }
    wp_send_json_success($data);
}
add_action('wp_ajax_seopress_request_alexa_rank', 'seopress_request_alexa_rank');
...
function seopress_request_google_analytics() {
    ...
    if (seopress_google_analytics_auth_option() !='' && seopress_google_analytics_auth_token_option() !='') {
        ...
        if ( false === ( $seopress_results_google_analytics_cache = get_transient( 'seopress_results_google_analytics' ) ) ) {
            ...
            //Language
            $ga_language = $service->data_ga->get($ga_account, $period,'today','ga:users', array('dimensions' => 'ga:language', 'sort'=> '-ga:users'));
            
            //Country
            $ga_country = $service->data_ga->get($ga_account, $period,'today','ga:users', array('dimensions' => 'ga:country', 'sort'=> '-ga:users'));
            
            //Desktop / mobile / tablet
            $ga_deviceCategory = $service->data_ga->get($ga_account, $period,'today','ga:users', array('dimensions' => 'ga:deviceCategory', 'sort'=> '-ga:users'));
            
            //OS
            $ga_operatingSystem = $service->data_ga->get($ga_account, $period,'today','ga:users', array('dimensions' => 'ga:operatingSystem', 'sort'=> '-ga:users'));
            
            //Browser
            $ga_browser = $service->data_ga->get($ga_account, $period,'today','ga:users', array('dimensions' => 'ga:browser', 'sort'=> '-ga:users'));
            
            //Screen Resolution
            $ga_screenResolution = $service->data_ga->get($ga_account, $period,'today','ga:users', array('dimensions' => 'ga:screenResolution', 'sort'=> '-ga:users'));
            
            //Social Networks
            $ga_socialNetwork = $service->data_ga->get($ga_account, $period,'today','ga:users', array('dimensions' => 'ga:socialNetwork','sort'=> '-ga:users'));

            //Channels
            $ga_channelGrouping = $service->data_ga->get($ga_account, $period,'today','ga:users', array('dimensions' => 'ga:channelGrouping'));
            
            //Organic searches: keywords
            $ga_keyword = $service->data_ga->get($ga_account, $period,'today','ga:organicSearches', array('dimensions' => 'ga:keyword'));
            
            //Sources/support
            $ga_source = $service->data_ga->get($ga_account, $period,'today','ga:users', array('dimensions' => 'ga:source','sort'=> '-ga:users'));
            
            //Referrer
            $ga_fullReferrer = $service->data_ga->get($ga_account, $period,'today','ga:users', array('dimensions' => 'ga:fullReferrer','sort'=> '-ga:users'));
            
            //Medium
            $ga_medium = $service->data_ga->get($ga_account, $period,'today','ga:users', array('dimensions' => 'ga:medium','sort'=> '-ga:users'));
            
            //Content pages
            $ga_contentpageviews = $service->data_ga->get($ga_account, $period,'today','ga:pageviews', array('dimensions' => 'ga:pageTitle','sort'=> '-ga:pageviews'));
            
            //Total Events
            $ga_totalEvents = $service->data_ga->get($ga_account, $period,'today','ga:totalEvents', array('dimensions' => 'ga:date', 'sort' => 'ga:date'));
            
            //Total Unique Events
            $ga_uniqueEvents = $service->data_ga->get($ga_account, $period,'today','ga:uniqueEvents', array('dimensions' => 'ga:date', 'sort' => 'ga:date'));
            
            //Event category
            $ga_eventCategory = $service->data_ga->get($ga_account, $period,'today','ga:totalEvents', array('dimensions' => 'ga:eventCategory', 'sort' => '-ga:totalEvents'));
            
            //Event action
            $ga_eventAction = $service->data_ga->get($ga_account, $period,'today','ga:totalEvents', array('dimensions' => 'ga:eventAction', 'sort' => '-ga:totalEvents'));
            
            //Event label
            $ga_eventLabel = $service->data_ga->get($ga_account, $period,'today','ga:totalEvents', array('dimensions' => 'ga:eventLabel', 'sort' => '-ga:totalEvents'));
