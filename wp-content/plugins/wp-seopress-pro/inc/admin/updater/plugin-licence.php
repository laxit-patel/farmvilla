...
function seopress_pro_license_page() {
	...
	?>
		...
		<form class="seopress-option" method="post" action="<?php echo admin_url('options.php'); ?>">
			...
			<p>
				<?php _e('Trouble to activate your license key? Watch our video guide by clicking the <strong>Help tab button</strong> in the top right corner of this page.','wp-seopress-pro'); ?><br><br>
...
function seopress_activate_license() {
	...
	if( isset( $_POST['seopress_license_activate'] ) ) {
		...
		// Call the custom API.
		$response = wp_remote_post( STORE_URL_SEOPRESS, array( 'timeout' => 15, 'sslverify' => false, 'body' => $api_params ) );
